...
- `customer_numeric_id` is `NOT NULL`, unique, and DB-generated on creation (`trigger_set_customer_numeric_id` + `generate_customer_numeric_id()` + a global `customer_sequences` counter table, mirroring how `order_code` auto-increments per branch/year). It is shown read-only on both Add Customer and Edit Customer — no role, including admin, can change it from the UI, and `updateCustomer` never writes to the column.
