...
export async function GET(
  ...
  try {
    const expense = await getEmployeeExpenseDetail(id, currentUser);

    if (!expense) return notFoundResponse();

    const [categories, employees, orders] = await Promise.all([
      getExpenseCategories("employee"),
      getExpenseEmployees(expense.branchId),
      getExpenseOrders(expense.branchId),
    ]);

    return NextResponse.json({
      success: true,
      data: { expense, options: { categories, employees, orders } },
    });
  } catch (error) {
    console.error("Failed to load employee expense detail", error);
...
export async function PATCH(
  ...
  try {
    ...
    const parsed = updateEmployeeExpenseSchema.safeParse(body);
      ...
      return NextResponse.json(
        {
          ...
          fieldErrors: getUpdateEmployeeExpenseFieldErrors(parsed.error),
        },
        { status: 400 },
      );
    }

    await updateEmployeeExpense(currentUser, id, parsed.data);
    ...
    console.error("Employee expense update failed", error);
...
export async function DELETE(
  ...
  try {
    await deleteEmployeeExpense(currentUser, id);
    ...
    console.error("Employee expense delete failed", error);
