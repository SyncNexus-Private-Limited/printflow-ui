...
import type { DeleteBusinessExpenseApiResponse } from "@/lib/expenses/types";
import { formatCurrency, formatDate } from "@/lib/utils/format";

type BusinessExpenseDeleteDialogProps = {
  expense: {
    id: string;
    title: string | null;
    category: string;
...
export function BusinessExpenseDeleteDialog({
  expense,
  onClose,
  onSuccess,
}: BusinessExpenseDeleteDialogProps) {
  ...
  async function handleConfirm() {
    ...
    try {
      const res = await fetch(`/api/expenses/business/${expense.id}`, {
        method: "DELETE",
      });
      const data = (await res.json().catch(() => null)) as DeleteBusinessExpenseApiResponse | null;
      ...
      {expense ? (
        <div className="rounded-2xl border border-[rgb(var(--border)/0.72)] bg-[rgb(var(--muted)/0.4)] px-4 py-3">
          ...
          <p className="mt-0.5 text-xs text-[rgb(var(--muted-foreground))]">
            {expense.branchName ?? "Branch"} &middot; {expense.category} &middot;{" "}
