...
WITH legacy_categories AS (
  SELECT
    ...
    CASE
      WHEN bool_or(source = 'branch') AND bool_or(source = 'employee') THEN 'both'
...
WHERE NOT EXISTS (
  SELECT 1
  FROM expense_categories expense_categories_existing
  WHERE lower(expense_categories_existing.code) = lower(legacy_categories.code)
);

ALTER TABLE branch_expenses RENAME COLUMN name TO title;
...
FROM expense_categories AS expense_category
WHERE lower(expense_category.code) = regexp_replace(
  regexp_replace(lower(btrim(branch_expense.category)), '[^a-z0-9]+', '_', 'g'),
  '^_+|_+$',
  '',
  'g'
);
...
ALTER TABLE branch_expenses
  ADD CONSTRAINT branch_expenses_title_not_blank
...
SET
  ...
  title = COALESCE(NULLIF(btrim(employee_expense.remarks), ''), 'Employee Expense'),
  expense_date = employee_expense.created_at::date,
  category_id = expense_category.id
FROM users AS user_record,
  expense_categories AS expense_category
WHERE user_record.id = employee_expense.user_id
  AND lower(expense_category.code) = regexp_replace(
    regexp_replace(lower(btrim(employee_expense.category)), '[^a-z0-9]+', '_', 'g'),
    '^_+|_+$',
    '',
    'g'
  );
...
ALTER TABLE employee_expenses
  ADD CONSTRAINT employee_expenses_title_not_blank
  CHECK (btrim(title) <> '');
...
CREATE TABLE expense_attachments (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT expense_attachments_exactly_one_owner
    CHECK (num_nonnulls(branch_expense_id, employee_expense_id) = 1)
);

CREATE INDEX idx_branch_expenses_branch_id_expense_date
ON branch_expenses (branch_id, expense_date DESC);

CREATE INDEX idx_branch_expenses_category_id
ON branch_expenses (category_id);

CREATE INDEX idx_branch_expenses_order_vendor_id
ON branch_expenses (order_vendor_id);

CREATE INDEX idx_employee_expenses_branch_id_expense_date
ON employee_expenses (branch_id, expense_date DESC);

CREATE INDEX idx_employee_expenses_user_id_expense_date
ON employee_expenses (user_id, expense_date DESC);

CREATE INDEX idx_employee_expenses_category_id
ON employee_expenses (category_id);

CREATE INDEX idx_expense_attachments_branch_expense_id
ON expense_attachments (branch_expense_id);

CREATE INDEX idx_expense_attachments_employee_expense_id
ON expense_attachments (employee_expense_id);
...
UPDATE branch_expenses AS branch_expense
SET category = expense_category.code
FROM expense_categories AS expense_category
WHERE expense_category.id = branch_expense.category_id;

ALTER TABLE branch_expenses DROP CONSTRAINT IF EXISTS branch_expenses_title_not_blank;
...
ALTER TABLE branch_expenses RENAME COLUMN title TO name;
...
SET category = expense_category.code
FROM expense_categories AS expense_category
WHERE expense_category.id = employee_expense.category_id;

ALTER TABLE employee_expenses DROP CONSTRAINT IF EXISTS employee_expenses_title_not_blank;
