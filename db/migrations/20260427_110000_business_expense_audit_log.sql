...
-- Append-only audit trail for business expense create / update / delete actions.
...
COMMENT ON TABLE business_expense_audit_logs IS
  'Append-only audit trail for business expense create/update/delete actions. '
  'expense_id is not a FK so rows survive expense deletion.';

COMMENT ON COLUMN business_expense_audit_logs.snapshot IS
  'Full row state at the time of the action. '
  'For update/delete: the before-state. For create: the inserted state.';

COMMENT ON COLUMN business_expense_audit_logs.changed_fields IS
  'For update actions: a JSON object mapping field names to {from, to} pairs. NULL for create/delete.';

CREATE INDEX idx_business_expense_audit_logs_expense_id
  ON business_expense_audit_logs (expense_id, changed_at DESC);
