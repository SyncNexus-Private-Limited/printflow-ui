...
CREATE TABLE offers (
  ...
  CHECK (
    ...
    OR
    (offer_type = 'buy_x_get_y' AND discount_value IS NULL AND buy_quantity IS NOT NULL AND get_quantity IS NOT NULL)
  )
);
...
CREATE INDEX idx_offer_audit_logs_offer_created_at
  ON offer_audit_logs(offer_id, created_at DESC);
