...
ALTER TABLE order_vendors
  ADD CONSTRAINT order_vendors_status_check
    CHECK (status IN ('assigned', 'in_progress', 'received', 'cancelled'));
...
SET action = 'update'
WHERE action IN ('vendor_assigned', 'vendor_payment_recorded');
