...
COMMENT ON TABLE branch_audit_logs IS
  'Append-only audit trail for branch management actions.';

CREATE INDEX IF NOT EXISTS idx_branch_audit_logs_branch_id
  ON branch_audit_logs (branch_id);

CREATE INDEX IF NOT EXISTS idx_branch_audit_logs_action
  ON branch_audit_logs (action);

CREATE INDEX IF NOT EXISTS idx_branch_audit_logs_changed_by
  ON branch_audit_logs (changed_by);

CREATE INDEX IF NOT EXISTS idx_branch_audit_logs_created_at
  ON branch_audit_logs (created_at DESC);

CREATE INDEX IF NOT EXISTS idx_branch_audit_logs_branch_created_at
  ON branch_audit_logs (branch_id, created_at DESC);
