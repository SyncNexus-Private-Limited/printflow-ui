...
COMMENT ON COLUMN user_auth.password_changed_at IS 'Timestamp of the last password change (admin reset or self-service). ' 'NULL means the password has not been changed since account creation.';
...
CREATE INDEX idx_user_audit_logs_user_id ON user_audit_logs (user_id, changed_at DESC);
